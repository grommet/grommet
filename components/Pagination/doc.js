...
var doc = function doc(Pagination) {
  ...
  DocumentedPagination.propTypes = _extends({}, _propTypes.genericProps, {
    ...
    numberMiddlePages: _reactDesc.PropTypes.number.description("The number of pagination buttons visible in the middle of the \n        controls.").defaultValue(3),
    onChange: _reactDesc.PropTypes.func.description("Function called when the user clicks a page or arrow button. The \n        single argument is an event containing the target page via \n        `event.page`, and the startIndex and endIndex for items contained \n        in the target page via `event.startIndex` and `event.endIndex`, \n        respectively.").defaultValue(undefined),
    page: _reactDesc.PropTypes.number.description("The default page. If used with onChange, it can be used to control the \n        active page via state.").defaultValue(undefined),
    size: _reactDesc.PropTypes.oneOf(['small', 'medium', 'large']).description('Specifies what size the pagination control buttons should be.').defaultValue('medium'),
    step: _reactDesc.PropTypes.number.description('The number of items per page.').defaultValue(10)
  });
