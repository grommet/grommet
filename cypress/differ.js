...
const differ = (file) => {
  execSync(`./node_modules/.bin/blink-diff --threshold-type percent --threshold 10 --no-copy --output "${constructPath(DiffScreenshots, file)}" "${constructPath(BaseScreenshots, file)}" "${constructPath(NewScreenshots, file)}"`)
}
