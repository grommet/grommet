...
- **Context & Hooks:** When a parent component needs to share state or callbacks with deeply nested children (e.g., `Accordion` communicating open/close state to each `AccordionPanel`), use a React context. Expose a named hook (e.g., `useAccordion()`) as the public access point rather than exporting the context object directly — this keeps the internal shape private and gives a cleaner API. The hook calls `React.useContext` internally. (See Section 1 for where context files live.)
