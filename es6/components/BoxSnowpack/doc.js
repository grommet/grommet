...
export var doc = function doc(BoxSnowpack) {
  var DocumentedBoxSnowpack = describe(BoxSnowpack).availableAt(getAvailableAtBadge('Box', 'Layout')).description("A container that lays out its contents in one direction. Box\n      provides CSS flexbox capabilities for layout, as well as general\n      styling of things like background color, border, and animation.").usage("import { BoxSnowpack } from 'grommet';\n<BoxSnowpack />").intrinsicElement('div');
  DocumentedBoxSnowpack.propTypes = _extends({}, genericProps, {
