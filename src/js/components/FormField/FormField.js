...
class FormField extends Component {
  render() {
    const { children, error, focus, help, htmlFor, label, style, theme, ...rest } = this.props;
    ...
    if (border) {
      ...
      contents = (
        <Box
          ...
          border={border.position === 'inner' ? { ...border, side: border.side || 'bottom', color: borderColor } : undefined}
        >
          {normalizedChildren}
        </Box>
      );
    ...
    return (
      <Box
        border={border && border.position === 'outer' ? { ...border, color: borderColor } : undefined}
      ...
      >
        {label || help ? (
          <Box margin={{ vertical: 'xsmall', horizontal: 'small' }} gap="xsmall">
            ...
            {help ? (
              <Text {...formField.help} color={formField.help.color[theme.dark ? 'dark' : 'light']}>
                {help}
              </Text>
            ) : (
              undefined
        ...
        {error ? (
          <Box margin={{ vertical: 'xsmall', horizontal: 'small' }}>
            <Text {...formField.error} color={formField.error.color[theme.dark ? 'dark' : 'light']}>
              {error}
            </Text>
          </Box>
        ) : (
