...
class Chart extends Component {
  ...
  render() {
    const { color, onClick, onHover, overflow, round, size, theme, thickness, type, ...rest } = this.props;
    delete rest.values;
    ...
    const width = sizeWidth === 'full' ? containerWidth : parseMetricToNum(theme.global.size[sizeWidth]);
    const height = sizeHeight === 'full' ? containerHeight : parseMetricToNum(theme.global.size[sizeHeight]);
    const strokeWidth = parseMetricToNum(theme.global.edgeSize[thickness]);
    const scale = [width / (bounds[0][1] - bounds[0][0]), height / (bounds[1][1] - bounds[1][0])];
    const viewBox = overflow ? `0 0 ${width} ${height}` : `-${strokeWidth / 2} -${strokeWidth / 2} ${width + strokeWidth} ${height + strokeWidth}`;
