...
const renderBars = (valuesState, boundsState, scale, height) =>
  (valuesState || []).map((valueArg, index) => {
    const { label, onHover, value, ...rest } = valueArg;

    const key = `p-${index}`;
    const bottom = value.length === 2 ? boundsState[1][0] : value[1];
    const top = value.length === 2 ? value[1] : value[2];
    if (top !== 0) {
      const d =
        `M ${(value[0] - boundsState[0][0]) * scale[0]},` +
        `${height - (bottom - boundsState[1][0]) * scale[1]}` +
        ` L ${(value[0] - boundsState[0][0]) * scale[0]},` +
        `${height - (top - boundsState[1][0]) * scale[1]}`;
...
const renderLine = (
  ...
  let d = '';
  (valuesState || []).forEach(({ value }, index) => {
    d +=
      `${index ? ' L' : 'M'} ${(value[0] - boundsState[0][0]) * scale[0]},` +
      `${height - (value[1] - boundsState[1][0]) * scale[1]}`;
  });
...
const renderArea = (
  ...
  (valuesState || []).forEach(({ value }, index) => {
    const top = value.length === 2 ? value[1] : value[2];
    d +=
      `${!index ? 'M' : ' L'} ${(value[0] - boundsState[0][0]) * scale[0]},` +
      `${height - (top - boundsState[1][0]) * scale[1]}`;
  });
  (valuesState || []).reverse().forEach(({ value }) => {
    const bottom = value.length === 2 ? boundsState[1][0] : value[1];
    d +=
      ` L ${(value[0] - boundsState[0][0]) * scale[0]},` +
      `${height - (bottom - boundsState[1][0]) * scale[1]}`;
