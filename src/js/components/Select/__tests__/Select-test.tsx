...
import { Select, SelectExtendedProps } from '..';
...
describe('Select', () => {
  ...
  // @ts-ignore
  const values: SelectExtendedProps['value'][] = [0, null];
  ...
  test('select an option', () => {
    ...
    if (!(select instanceof HTMLButtonElement)) {
      fail("expected select to be of type 'HTMLButtonElement'");
    }
    expect(container.firstChild).toMatchSnapshot();
    fireEvent.click(getByPlaceholderText('test select'));
  ...
  test('select an option with enter', () => {
    ...
    fireEvent.click(getByPlaceholderText('test select'));
    const selectDiv = document.getElementById('test-select__select-drop');
    expect(selectDiv).not.toBeNull();
    // verify that keyboard navigation is working
    fireEvent.keyDown(selectDiv!, {
      key: 'Down',
      keyCode: 40,
      which: 40,
    });
  ...
  test(`renders styled select options backwards compatible with legacy
    ...
    const optionButtonChild = optionButton!.firstElementChild;
    expect(optionButtonChild).not.toBeNull();
    const style = window.getComputedStyle(optionButtonChild!);
  ...
  test(`renders styled select options combining select.options.box &&
    ...
    style = window.getComputedStyle(optionButtonChild!);
  ...
  test('select an option then select a different option', () => {
    ...
    // select first option
    const firstOptionButton = document
      .getElementById('test-select__drop')
      ?.querySelector('button');
    expect(firstOptionButton).toBeDefined();
    ...
    const secondOptionButton = document
      .getElementById('test-select__drop')
      ?.querySelectorAll('button')[1];
    expect(secondOptionButton).toBeDefined();
