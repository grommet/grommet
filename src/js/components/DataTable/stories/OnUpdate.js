...
const buildGroups = () => {
  ...
  const nextGroups = groupKeys.map(id => {
    const members = data.filter( item => item.location === id)
    ...
    if (group.members.length > 0) {
      if (group.id) {
        ...
        const selectedMembers =
          keys.filter(key => selected.includes(key)).length;
...
export const OnUpdateDataTable = () => {
  ...
  const onGroupSelect = useCallback((selected, row, groupBySelected) => {
    let nextSelected;
