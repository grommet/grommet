...
if (process.env.NODE_ENV !== 'production') {
  PropType = {
    ...
    messages: PropTypes.shape({
      // NOTE: `ascending`/`descending` are no longer read by DataTable's
      // sort UI as of the #7924 accessibility fix (sort status is now
      // communicated via `sortable`/`sortedAscending`/`sortedDescending`
      // below). Kept here to avoid a breaking change for consumers who
      // may have overridden them; raised for team discussion on a formal
      // deprecation/removal in a future release.
