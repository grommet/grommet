...
export default class DateTimeDrop extends Component {
  ...
  _renderGrid() {
    ...
    const rows = dateRows.map((row, rowIndex) => {
      const days = row.map((date, columnIndex) => {
        const classes = classnames(`${CLASS_ROOT}__day`, {
    ...
    const gridClasses = classnames(`${CLASS_ROOT}__grid`, {
  ...
  _renderCalendar() {
    ...
    const grid = format.match(/D/) ? this._renderGrid() : <span key="grid" />;
