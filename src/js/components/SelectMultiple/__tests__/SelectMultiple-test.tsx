...
describe('SelectMultiple', () => {
  test('should not have accessibility violations', async () => {
    ...
    const results = await axe(container, {
      rules: {
        /* This rule is flagged because when no value is selected,
        SelectMultiple renders a readOnly, non-focusable placeholder
        TextInput within a DropButton (see DefaultSelectTextInput).
        According to Dequeue and WCAG 4.1.2 "interactive controls must not
        have focusable descendants". Jest-axe is assuming that the input
        is focusable and since the input is a descendant of the button the
        rule is flagged. However, the TextInput is built so that it is
        read only and cannot receive focus. SelectMultiple is accessible
