...
const Node = ({ id, ...rest }) => <Box id={id} basis="xxsmall" margin="small" pad="medium" round="small" background="light-4" {...rest} />;
  ...
  render() {
    ...
    const content = [
      ...
      <Box key="input" gap="small">
        ...
        <CheckBox name="check" checked={checkBox} label="CheckBox" onChange={event => this.setState({ checkBox: event.target.checked })} />
        <CheckBox
          name="toggle"
          toggle
          checked={checkBox}
      ...
      <Box key="measure" gap="medium">
        <Chart type="bar" round size="small" values={[{ value: [10, 20] }, { value: [20, 30] }, { value: [30, 15] }]} />
        <Meter type="bar" round size="small" background="light-3" values={[{ value: 30 }]} />
      </Box>,
    ...
    return (
      <React.Fragment>
        <Grommet theme={grommet}>
          <Box direction="row-responsive" gap="medium" justify="end" align="center" margin="small">
            <Box basis="small">
              <Select
                plain
<<<<<<< HEAD
...
=======
          <Box pad="medium" background={theme.global.colors.background || theme.global.colors.white}>
            {Grid.available ? (
