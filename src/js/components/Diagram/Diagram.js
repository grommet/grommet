...
const computeMidPoint = (fromPoint, toPoint) => [
  fromPoint[0] > toPoint[0] ? toPoint[0] + (fromPoint[0] - toPoint[0]) / 2 : fromPoint[0] + (toPoint[0] - fromPoint[0]) / 2,
  fromPoint[1] > toPoint[1] ? toPoint[1] + (fromPoint[1] - toPoint[1]) / 2 : fromPoint[1] + (toPoint[1] - fromPoint[1]) / 2,
];
...
const COMMANDS = {
  ...
  direct: (fromPoint, toPoint, offset) => `M ${fromPoint[0] + offset},${fromPoint[1] + offset} ` + `L ${toPoint[0] + offset},${toPoint[1] + offset}`,
...
class Diagram extends Component {
  ...
  placeConnections() {
    ...
    const connectionPoints = connections.map(({ anchor, fromTarget, toTarget }) => {
      ...
      if (fromElement && toElement) {
        ...
        const fromPoint = [fromRect.x - containerRect.x || 0, fromRect.y - containerRect.y || 0];
        const toPoint = [toRect.x - containerRect.x || 0, toRect.y - containerRect.y || 0];
        if (anchor === 'vertical') {
  ...
  render() {
    ...
    let paths;
    if (connectionPoints) {
      paths = connections.map(({ anchor, color, offset, round, thickness, type, ...connectionRest }, index) => {
        let path;
        ...
        if (points) {
          ...
          const d = COMMANDS[type || 'curved'](points[0], points[1], offsetWidth, anchor);
          const strokeWidth = thickness ? parseMetricToNum(theme.global.edgeSize[thickness] || thickness) : 1;
    ...
    return (
      <StyledDiagram ref={this.svgRef} viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="xMinYMin meet" {...rest}>
        <g>{paths}</g>
      </StyledDiagram>
    );
  }
