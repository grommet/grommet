...
const LazyDrop = () => {
  const [pad, setPad] = useState('small');

  const topLeftTargetRef = useRef(null);

  const topRightTargetRef = useRef(null);

  const bottomLeftTargetRef = useRef(null);

  const bottomRightTargetRef = useRef(null);
