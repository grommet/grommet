...
const Tag = forwardRef(
  ({ name, value, size, truncate, truncateNumChars=16, onRemove, onClick, ...rest }, ref) => {
    ...
    const displayName = truncate && name.length > truncateNumChars? name.substring(0,truncateNumChars-3)+'...' : name;
    const displayValue = truncate && value.length > truncateNumChars? value.substring(0,truncateNumChars-3)+'...' : value;
    const tipValue1 = value.length < 128? value : value.substring(0,128);
    const tipValue2 = value.length < 128? '' : value.substring(128);
    const shouldDisplayTip  = truncate && (name.length > truncateNumChars || value.length > truncateNumChars);
    const tipContent=(<Box direction='column'>
      <Text size={size}>{name+' : '}</Text>
      {tipValue1 && <Text weight='bold' size={size}>{tipValue1}</Text>}
      {tipValue2 && <Text weight='bold' size={size}>{tipValue2}</Text>}
    </Box>)
